    ...
    def optimise(self, x0):
        ...
        for i in range(self.max_iter):
            ...
            if norm_grad_f_x < self.tol:
                ...
                print('TERMINATED')
                print('------------------------------------------------------------------------------------------')
                print(f"Iteration {i:4}: x = [{x_str}], f(x) = {f_x:10.6f}, grad norm = {norm_grad_f_x:10.6e}")
                break
            
            direction = -grad_f_x
            step_size = self.backtracking_line_search(x, direction)
            
            if self.verbose:
                f_x = self.func(x)
                x_str = ", ".join([f"{xi:8.4f}" for xi in x])
                print(f"Iteration {i:4}: x = [{x_str}], f(x) = {f_x:10.6f}, grad norm = {norm_grad_f_x:10.6e}, step size = {step_size:8.6f}")
